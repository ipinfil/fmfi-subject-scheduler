...
def predmety():
    ...
    for row in final_rows:
        ...
        line = f"predmet({repair(tds[0].contents[0][2:])}, \"{name}\", {year}, {sem}, {cr})."
        print(line)
...
def predmety_ucitelia():
    ...
    for row in final_rows:
        ...
        for n in names:
            p = False
            if not ucitelia_map.get(ucitel_id(n) + repair(tds[0].contents[0][2:]), False):
                p = True

            ucitelia_map[ucitel_id(n) + repair(tds[0].contents[0][2:])] = True

            if p:
                print(f"predmet_ucitel({repair(tds[0].contents[0][2:])}, {ucitel_id(n)}).")

def ucitel_id(ucitel):
    return unicodedata.normalize('NFKD', ucitel.lower().replace(". ", "")).encode('ASCII', 'ignore').decode().replace(" ", "")
